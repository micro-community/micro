...
import (
	...
	cliutil "github.com/micro-community/micro/v3/cmd/cli/util"
	protoDebug "github.com/micro-community/micro/v3/proto/debug"
...
func QueryHealth(c *cli.Context, args []string) ([]byte, error) {
	...
	req := client.NewRequest(args[0], "Debug.Health", &protoDebug.HealthRequest{})

	// if the address is specified then we just call it
	if addr := c.String("address"); len(addr) > 0 {
		rsp := &protoDebug.HealthResponse{}
